...
def test_upload_from_url_file_type_detection():
    ...
    assert upload_client._detect_file_type_from_url_and_content("test.jpg", None) == EUploadFileType.Image
    assert upload_client._detect_file_type_from_url_and_content("test.png", None) == EUploadFileType.Image
    assert upload_client._detect_file_type_from_url_and_content("test.mp4", None) == EUploadFileType.Video
    assert upload_client._detect_file_type_from_url_and_content("test.pdf", None) == EUploadFileType.Pdf
    assert upload_client._detect_file_type_from_url_and_content("test.txt", None) == EUploadFileType.File
    
    # Test content type detection
    assert upload_client._detect_file_type_from_url_and_content("noext", "image/jpeg") == EUploadFileType.Image
    assert upload_client._detect_file_type_from_url_and_content("noext", "video/mp4") == EUploadFileType.Video
    assert upload_client._detect_file_type_from_url_and_content("noext", "application/pdf") == EUploadFileType.Pdf
    assert upload_client._detect_file_type_from_url_and_content("noext", "text/plain") == EUploadFileType.File
    
    # Test default fallback
    assert upload_client._detect_file_type_from_url_and_content("noext", "unknown/type") == EUploadFileType.File
...
def test_upload_multiple_files_from_urls(client):
    ...
    urls_and_types = [
        ...
        ("https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf", EUploadFileType.Pdf, "dummy.pdf"),
    ]
...
def test_create_task_with_external_file_from_url(client):
    ...
    assert created_task["group"] == TEST_GROUP_ID
    ...
    print(f"✅ Successfully created task '{created_task['name']}' with external file from URL")
